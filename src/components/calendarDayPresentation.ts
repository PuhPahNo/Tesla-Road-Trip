 ...
 * metadata is separate so its outline can coexist with every day tone.
 */
export function calendarDayPresentation(
  ...
  const landmark = day.rating.places
    .filter((place) => place.type === 'landmark')
    .reduce<PlaceRating | undefined>(
      (best, place) =>
        !best ||
        place.rating > best.rating ||
        (place.rating === best.rating && place.sceneryScore > best.sceneryScore)
          ? place
          : best,
      undefined,
    )
