...
describe('ChargeQuest landing page', () => {
  it('invites visitors into the first quest, previews CORE, and shows the honest pre-trip status', async () => {
    ...
    const heroImage = screen.getByRole('img', {
      name: 'An open highway running through the painted desert',
    })
    expect(heroImage.closest('picture')).toBeTruthy()
    expect(heroImage.getAttribute('src')).toBe('/landing/desert-road-1280.jpg')
    expect(heroImage.getAttribute('srcset')).toContain('desert-road-960.jpg 960w')
    expect(heroImage.getAttribute('fetchpriority')).toBe('high')
    expect(heroImage.getAttribute('width')).toBe('1280')
    expect(heroImage.getAttribute('height')).toBe('1600')
    expect(document.querySelector('source[type="image/avif"]')?.getAttribute('srcset'))
      .toContain('desert-road-960.avif 960w')
    ...
    const chargerImage = screen.getByRole('img', { name: 'Tesla Superchargers illuminated at night' })
    expect(chargerImage.getAttribute('loading')).toBe('lazy')
    expect(chargerImage.closest('picture')).toBeTruthy()
